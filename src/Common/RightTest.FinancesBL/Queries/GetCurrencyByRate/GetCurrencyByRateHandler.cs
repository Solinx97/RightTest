using MediatR;
using Microsoft.EntityFrameworkCore;
using RightTest.FinancesBL.DTOs;
using RightTest.FinancesDAL.Entities;
using RightTest.FinancesDAL.Persistent;

namespace RightTest.FinancesBL.Queries.GetCurrencyByRate;

internal class GetCurrencyByRateHandler(FinancesContext context) : IRequestHandler<GetCurrencyByRateQuery, IEnumerable<CurrencyDto>>
{
    private readonly FinancesContext _context = context;

    public async Task<IEnumerable<CurrencyDto>> Handle(GetCurrencyByRateQuery request, CancellationToken cancelationToken)
    {
        var currencies = await _context.Set<Currency>()
            .AsNoTracking()
            .Where(x => x.Rate == request.Rate)
            .Select(x => new CurrencyDto(x.Id, x.Name, x.Rate))
            .ToListAsync(cancelationToken);

        return currencies;
    }
}
